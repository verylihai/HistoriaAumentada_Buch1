...
/// This just exposes the Sorting Layer / Order in SkinnedMeshRenderer since it's there
/// but not displayed in the inspector. Getting SkinnedMeshRenderer to render in front
...
public class SkinnedMeshRendererSortingEditor : Editor {

    private Editor defaultEditor;
    private SkinnedMeshRenderer skinnedMeshRenderer;
    private static bool showSorting = true;
    private string header = "Skinned Mesh Renderer Sorting";
    ...
    private static void OnLoad() {
        showSorting = EditorPrefs.GetBool("SkinnedMeshRendererSortingEditor.showSorting");
    }
    ...
    /*public override void OnInspectorGUI() {
        ...
        if (EditorGUI.EndChangeCheck())
            EditorPrefs.SetBool("SkinnedMeshRendererSortingEditor.showSorting", showSorting);

        if (showSorting) {
            EditorGUI.indentLevel++;

            EditorGUI.BeginChangeCheck();
            var newId = DrawSortingLayersPopup(skinnedMeshRenderer.sortingLayerID);
            if (EditorGUI.EndChangeCheck()) {
                sortingLayerIdProperty.intValue = newId;
            }

            EditorGUI.BeginChangeCheck();
            var order = EditorGUILayout.IntField("Sorting Order", skinnedMeshRenderer.sortingOrder);
            if (EditorGUI.EndChangeCheck()) {
